...
- ✅ 用户注册接口（`POST /v1/register`）

  - 邮箱唯一性验证
  - bcrypt 密码加密（cost=10）
  - 自动生成用户 ID

- ✅ 用户登录接口（`POST /v1/login`）
...
- ✅ `docs/API接口文档.md` - API 接口文档
  ...
  - 添加完整认证工作流
  - 更新错误处理说明
- ✅ 2025-12-06：统一 TODO API 与网关路径示例
  - 将文档中的 `/todos`、`/login` 等旧路径更新为 `/v1/todos`、`/v1/login`
  - 将网关示例统一为 `/api/v1/todos`，与 Gin/stdlib 网关实现保持一致
  - 校对并修复 `docs/API接口文档.md`、`docs/本地开发指南.md`、`docs/前端使用指南.md`、`docs/RBAC权限控制.md`、`docs/JWT认证系统.md` 及脚本输出中的访问地址说明
